...
select COUNT(role_name) as Êîëè÷åñòâî_QA from
roles r 
...
select COUNT(role_name) as Êîëè÷åñòâî_Middle from
roles r 
join roles_employee re on r.id = re.role_id 
join employees e on re.employee_id = e.id
where role_name like '%Middle%'
...
select COUNT(role_name) as Êîëè÷åñòâî_ðàçðàáîò÷èêîâ from
roles r 
join roles_employee re on r.id = re.role_id 
join employees e on re.employee_id = e.id
...
select SUM(monthly_salary) as Ôîíä_çï_ðàçðàáîò÷èêîâ from
salary s 
join employee_salary es on s.id = es.salary_id 
