...
-- 26. Вывести количество Middle специалистов.
	select COUNT(role_name) as Количество_Middle from
	roles r 
	join roles_employee re on r.id = re.role_id 
	join employees e on re.employee_id = e.id
	where role_name like '%Middle%'
...
-- 27. Вывести количество разработчиков
	select COUNT(role_name) as Количество_разработчиков from
	roles r 
	join roles_employee re on r.id = re.role_id 
	join employees e on re.employee_id = e.id
...
-- 28. Вывести фонд (сумму) зарплаты разработчиков.
   select SUM(monthly_salary) as Фонд_зп_разработчиков from
   salary s 
   join employee_salary es on s.id = es.salary_id 
